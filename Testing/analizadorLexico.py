...
def process(line):
    ...
    while i <= (len(line)):
        if(i < len(line)):
            ...
            try:
                # Valor almacena cuantos caracteres hay que retroceder luego de llegar al estado
                valor = estados[state](x)
                i = i - valor
            # No existe transicion para ese estado y ese caracter
            except KeyError:
                error.append(
                    '[' + repr(nroLinea) + ']' " Caracter(es) no reconocido(s) " + cadena)
                cadena = ""
        else:
            estados[state](x)
            break
...
def main():
    ...
    if bandera:
        # hay que pensar si puede haber mas de un error como tratarlo quizas imprimir un error por linea y enumerarlos
        error.append(
...
def inicializar(nombreArchivo,verboso_lexico=False):
    global nroLinea
    global archivo

    nroLinea = 0
    archivo = nombreArchivo
    args.verbose_mode = verboso_lexico
...
parser.add_argument("-verbose_mode", "-v",
                    help="Flag para modo Verboso con impresiones de control.", action='store_true')
parser.add_argument("-standalone", "-s",
                    help="Flag para funcionamiento por separado del aplicativo.", action='store_true')
